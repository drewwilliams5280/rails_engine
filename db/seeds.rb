...
puts "Creating merchant data."
CSV.foreach("./data/merchants.csv", headers: true) do |row|
  Merchant.create!(id: row['id'], name: row['name'], created_at: row['created_at'], updated_at: row['updated_at'])
end

puts "Creating item data."
CSV.foreach("./data/items.csv", headers: true) do |row|
  Item.create!(id: row['id'], name: row['name'], description: row['description'], unit_price: (row['unit_price'].to_f / 100), merchant_id: row['merchant_id'], created_at: row['created_at'], updated_at: row['updated_at'])
end
...
puts "Creating invoice item data."
CSV.foreach("./data/invoice_items.csv", headers: true) do |row|
  InvoiceItem.create!(id: row['id'], item_id: row['item_id'], invoice_id: row['invoice_id'], unit_price: (row['unit_price'].to_f / 100), quantity: row['quantity'], created_at: row['created_at'], updated_at: row['updated_at'])
end

puts "Creating transaction data."
